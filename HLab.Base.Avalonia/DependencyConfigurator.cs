...
namespace HLab.Base.Wpf
{
        ...
        where TClass : StyledElement
    {
        ...
        /// <returns></returns>
        public StyledProperty<TValue> RegisterAttached<T>() => AvaloniaProperty.Register<T,TValue>(
            _name,
            _propertyMetadata,
            _validateValueCallback
        );
        ...
        /// <returns></returns>
        public DependencyPropertyConfigurator<TClass,TValue> OnChange<TSender>(Action<TSender, Avalonia.DependencyPropertyChangedEventArgs<TValue>> action)
            where TSender : StyledElement
        {
            _propertyMetadata.PropertyChangedCallback += (sender, args) =>
            {
                ...
                {
                    action(typedSender, new Avalonia.DependencyPropertyChangedEventArgs<TValue>(args));
                }
                else
                {
                    if(sender==null)
                    {
                        action(null, new Avalonia.DependencyPropertyChangedEventArgs<TValue>(args));
                    }
                }
        ...
        /// <param name="action"></param>
        /// <returns></returns>
        public DependencyPropertyConfigurator<TClass, TValue> OnChange(Action<TClass, Avalonia.DependencyPropertyChangedEventArgs<TValue>> action)
