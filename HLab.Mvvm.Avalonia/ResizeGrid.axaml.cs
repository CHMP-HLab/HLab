...
public partial class ResizeGrid : UserControl
{
    ...
    {
        if (ThisWindow?.IsInitialized ?? false) InitializeWindowSource(sender, e);

        ThisWindow.Initialized += InitializeWindowSource;
    }
    ...
    public static readonly StyledProperty<object> NestedContentProperty = H.Property<object>().Register();
    ...
    {
        SendMessage(_hwnd/*.Handle*/, WmSyscommand, (IntPtr)(61440 + direction), IntPtr.Zero);
    }
    ...
    void InitializeWindowSource(object sender, EventArgs e)
    {
        ...
        var handle = platformImpl.Handle;
        _hwnd = handle.Handle;
        ...
        switch (msg)
        {
            case (int)WindowMessage.WM_NCHITTEST:
                try
                {
                    ...
                    var rect = new Rect(MaximizeButton.PointToScreen(new Point()).ToPointWithDpi(96.0), new Size(MaximizeButton.Width, MaximizeButton.Height));
    ...
    void Border_MouseDown(object? sender, PointerPressedEventArgs pointerPressedEventArgs)
    {
        if (sender is not Control { Tag: ResizeDirection dir }) return;

        ResizeWindow(dir);
    }

    public void DragWindow(object sender, PointerEventArgs e)
    {
